...
os.makedirs("genome_list/", exist_ok=True)
...
if snakemake.config['busco_output'] == "":
    ...
    for batch in generator:
        counter += 1
        with open(f"genome_list/batch_number_{counter}.txt", 'w') as f:
            f.write("\n".join(batch))
else:

    all_genomes = pd.read_csv(snakemake.config['all_genomes'], sep = '\t', index_col=0).reset_index()
    BUSCO_output = pd.read_csv(snakemake.config['busco_output'], sep = '\t', index_col=0)
    list_of_names_all_genomes = []
    os.makedirs("genome_list/by_cluster", exist_ok=True)
    #rename files to be more universal
    for i in range(len(all_genomes)):
        new_name = "_".join("".join(all_genomes.iloc[i]["genome_file"].rstrip(".gz").split(".")[:-1]).split("_")[:2])
    ...
    for n in range(len(BUSCO_output)):
        new_name = "_".join(BUSCO_output.iloc[n]['sample_name'].strip("[").strip("]").split("_")[:2])
    ...
    for i in range(1, len(big_table['cluster975'].unique())):
        ...
        try:
            if type(df.iloc[0]['lineage']) == str:
                txt_folder = "genome_list/by_cluster"
...
glob_argument = "genome_list/by_cluster/*.txt"
dictionary = {}
...
for sample in all_genomes_in_dir:
    sample_name = "_".join("".join("".join(sample.rstrip(".gz").split("/")[-1]).split(".")[:-1]).split("_")[:2]).strip("\n")
    try:
        lineage_of_sample = df_from_dict.columns[df_from_dict.isin([sample_name]).any()][0]
...
for key in list(dictionary_of_lineages.keys()):
    ...
    for batch in generator:
        ...
        with open(f"genome_list/batch_number_{counter}-{key}.txt", 'w') as f:
            f.write("\n".join(batch))
