...
for i in snakemake.input.fasta_dir:
    ...
    for path in glob.glob(glob_argument):
        ...
        if snakemake.config["busco_parameters"] == "cluster_analysis":
            ...
            shell(
                f"busco -i {path} -l {lineage} -m protein -o {sample_name} --out_path {snakemake.params.working_dir} -c {snakemake.threads} &> {snakemake.log}"
            )
        elif "auto-lineage-prok" in snakemake.config["busco_parameters"]:
            shell(
                f"busco -i {path} --auto-lineage-prok -m protein -o {sample_name} --out_path {snakemake.params.working_dir} -c {snakemake.threads} &> {snakemake.log}"
            )
        else:
