...
#     db_path = snakemake.config["database_folder"]
    ...
    #         #sample_name = "".join(path.split("/")[-1].split(".")[:-1]) OLD WAY, REMOVE IF NOT NEEDED
    #         sample_name = os.path.splitext(os.path.basename(path).rstrip(".gz"))[0]
    #         try:
    #             if snakemake.config["busco_parameters"] == "cluster_analysis":
    #                 lineage = "".join("".join(path.split("/")[-2]).split("-")[-1])
    #                 shell(
    #                     f"busco -i {path} -l {lineage} -f -m protein --offline -o {sample_name} --download_path {db_path} --out_path {snakemake.params.working_dir} -c {snakemake.threads} &> {snakemake.log}"
    #                 )
    #             elif "auto-lineage-prok" in snakemake.config["busco_parameters"]:
    #                 shell(
    #                     f"busco -i {path} --auto-lineage-prok -m protein -f --offline -o {sample_name} --download_path {db_path} --out_path {snakemake.params.working_dir} -c {snakemake.threads} &> {snakemake.log}"
    #                 )
    #             else:
    #                 lineage = snakemake.config["busco_parameters"]
    #                 shell(
    #                     f"busco -i {path} -l {lineage} -m protein -f -o {sample_name} --offline --download_path {db_path} --out_path {snakemake.params.working_dir} -c {snakemake.threads} &> {snakemake.log}"
    #                 )
    #         except:
...
if snakemake.config["busco_parameters"] == "cluster_analysis":
    lineage = "".join("".join(path.split("/")[-2]).split("-")[-1])
    shell(f"busco -i {path} -l {lineage} -f -m protein --offline -o {specific_name}--download_path {db_path} --out_path {out_dir} -c {snakemake.threads} &> {snakemake.log}")

elif "auto-lineage-prok" in snakemake.config["busco_parameters"]:
    shell(f"busco -i {path} --auto-lineage-prok -m protein -f --offline -o {specific_name} --download_path {db_path} --out_path {out_dir} -c {snakemake.threads} &> {snakemake.log}")

else:
    lineage = snakemake.config["busco_parameters"]
    shell(f"busco -i {path} -l {lineage} -m protein -f -o {specific_name} --offline --download_path {db_path} --out_path {out_dir} -c {snakemake.threads} &> {snakemake.log}")
